...
| `trash`              | `Object` | `{ enabled: true, retentionDays: 30, useSystemTrash: false }` | Where a file Vapor removes on this device goes (a deletion that arrived from the cloud, a `pull-only` mirror removal): the managed trash under `<vapor_dir>/trash/<profile>/`, listed and restored with `vapor trash list` and `vapor trash restore <id>`, purged after `retentionDays`. `useSystemTrash: true` tries the system trash first (the Finder's Trash on macOS) and falls back to the managed one. `enabled: false` unlinks. Applies live. |
