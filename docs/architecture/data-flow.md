...
8. Planner, hash, upload, and reconcile stages acquire strict throttle-gated work permits before starting; reconcile only starts in `IdleDrain`, yields on slice expiry or throttle changes, and clears compacted subtree boundaries after successful quiet completion. The one exception is the user's own request: `vapor sync-now` (the app's Sync now) admits the next whole-scope reconcile under any state but `Suspended`, spending that state's planner slot and read token, and the walk still yields on its slice budget; the request is spent when one reconcile completes. While a queued scan is held, `vapor status` reports `reconcile_state: waiting` with the throttle reason, so a machine in use never looks stuck without saying why.
...
- App issues controls: pause/resume, flush-now, sync-now, auto-launch toggle, excludes updates.
