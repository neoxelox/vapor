...
11. Provider selection is injected at runtime startup per profile, so daemon orchestration uses the provider trait boundary instead of hardcoding any cloud type in core engine state. `provider = "filesystem"` (default) selects the real filesystem provider; `provider = "google_drive"` selects `GoogleDriveProvider`. New backends onboard through the checklist in `provider-onboarding.md`.
