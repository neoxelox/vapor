...
- Two partial indexes, `idx_sync_index_conflict_copy` and `idx_queue_intents_conflict_copy`, hold only the rows whose path carries the conflict-copy marker (`~conflict-`). The unresolved-conflict count in `vapor status` reads them (the machine grammar of the name is checked in Rust on the handful of candidates), so the count costs the same on a hundred-thousand-row index as on an empty one. Indexes are created with `IF NOT EXISTS` on every open, so they need no schema version of their own; an existing database gains them at its next open.
