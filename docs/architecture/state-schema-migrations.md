...
- `name_aliases` records which cloud object owns a local name when two cloud names fold to one local name (case, Unicode normalization), so a conflict copy materialized for the second object stays tied to it.
