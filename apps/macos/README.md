...
- Settings/config surface includes `useGitIgnore`, `useVaporIgnore`, and user `ignoreRules` (embedded `.gitignore`-like text), persisted in `vapor.json` and exported to daemon launch env as `VAPOR_USE_GITIGNORE`, `VAPOR_USE_VAPORIGNORE`, and `VAPOR_IGNORE_RULES`.
