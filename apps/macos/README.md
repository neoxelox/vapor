...
- The menu bar mark has two faces. AppKit tints the template mark for the menu bar appearance; while `AppShellState.needsUserAction` holds (a parked question, an unresolved conflict, a crash-loop pause, a blocked login item, a restart-required setting, an unreadable config file, or the Error state the status text labels "Action required") the app shows the same mark filled with the brand colour (`VaporConstants.Brand`, Aerospace International Orange) as a non-template image, and the menu names the waiting questions and conflicts with the CLI commands that answer them. `MenuBarIcon.attention(from:)` rasterises that face once per scale from the template at launch.
- Under Error or Paused the status detail is the daemon's own run-state reason (the worst profile's `reason`, or its `suspended_reason` when one is set), such as a cloud root it cannot ensure, in place of the throttle reason. While the whole-scope scan is held by the throttle or running, the detail is the scan's own line (`reconcile_detail` from `vapor status --json`), so a machine in use says "waiting for an idle moment: user activity is active" instead of looking stuck. `AppShellState.syncDetail(for:status:)` is the one place that ranks the three.
- `Sync now`, in the menu and on the Dashboard, runs `vapor sync-now --json`: the scan is admitted under any throttle state but Suspended and deferred work is released. The Dashboard adds a one-line hint while a scan is waiting. The control is shown only while the daemon answers status.
- `Restart sync` acts on the registered LaunchAgent only. With Start Vapor at login off there is none; the control is disabled with a hint, and a `not_installed` answer from `vapor service restart` leaves the surface at Stopped rather than Error.
- The app never writes `cloudSyncDirectory` unless the user set one: the daemon resolves the unset key to the provider's default, which differs between a local folder playing the cloud and a real cloud provider.
...
- The app icon has two sources, both documented in `assets/README.md`: `package.sh` compiles the Icon Composer document `assets/macos/Vapor.icon/` with `actool` into `Contents/Resources/Assets.car` (what macOS 26 renders, with `CFBundleIconName` in the Info.plist) and packs `assets/macos/Vapor.iconset/` into `Vapor.icns` with `iconutil` as the flat fallback behind `CFBundleIconFile`. The same `actool` call compiles `assets/macos/Vapor.xcassets/`, whose `AccentColor` set is the brand orange, and the Info.plist names it in `NSAccentColorName`, so the packaged app's controls take the brand colour whenever the user's system accent is the default multicolour (a user who picked another accent keeps theirs, as the HIG expects). The menu bar status item shows the template mark from `assets/macos/menubar/`, tinted by AppKit for the current appearance.
