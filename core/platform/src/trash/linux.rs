//! Linux `TrashBin` stub: refuses with `Unsupported` so the daemon keeps
//! what it removes in its managed trash. The freedesktop trash lands
//! with the Linux surface.
